...
class BisectionHint:
    help_statement = "Divide the range i half. Cost : up to half the range"
...
class WarmColdHint:
    cost = 20
    help_statement = f"Warm is last guess is +. 15% of the answer. Cold otherwise. Cost: {cost}"
    ...
    def apply(self):
        if not self.game.prev_guess:
            msg = print("You must make at last one guess before using this hint. ")
            return msg, None

        if self.game.answer + self.game.num_range[1] * 1.15 >= self.game.prev_guess <= self.game.answer + self.game.num_range[1] * .15:
            temp = "Warm"
