...
from number_guessing_game.hint import *


class NumberGuessingGame:
    def __init__(self, num_range=(0, 10)):
    ...
    def run(self):
        while True:
            user_input = input(
                f"Guess a number between {self.num_range[0]} and {self.num_range[1]},or Type 'h' to use a hint. ")
            if user_input == 'h':
                self.get_user_hint()
            ...
            if correct:
                print(f"Congrats! you win! Your final current score is {self.points}) ")
                return
            else:
